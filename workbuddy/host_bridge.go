...
func hostHTTPDo(req *http.Request) (*hostHTTPResponse, error) {
	...
	// Windows stack movement mitigation: nested host calls during synchronous RPCs
	// (such as model.for_auth or management.handle) cause the host stack to move,
	// rendering the stack response pointer dangling and causing "unexpected end
	// of JSON input". Bypass host bridge on Windows and make direct HTTP calls.
